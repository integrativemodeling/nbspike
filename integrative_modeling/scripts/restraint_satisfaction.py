...
def _check_cluster(nanobody, rmf_fn, datadir, cluster_dir, outdir, n_threads):
    # topology file
    ...
    xl_fn = {nanobody: None}
    if has_XL:
        xl_fn[nanobody] = os.path.join(datadir, "xl", nanobody + ".csv")
        xl_cutoff = config.get("XL_CUTOFF", 30.0) + \
                    config.get("DELTA_XL_CUTOFF", 5.0)
        
        rs_xl = rs.CrosslinkSatisfaction(topology_file=topology_fn,
                                    rmf_file=rmf_fn,
        ...
        rs_xl.add_data(xl_fn, cutoff=xl_cutoff)
        rs_xl.run()
    ...
    rs_ep.render_binary_docking_epitope(XL_file=xl_fn[nanobody],
...
for i in range(n_clusters):
    ...
    _check_cluster(nanobody, rmf_fn, datadir, 
